...
<head>
    ...
    <title>@yield('title', 'لوحة التحكم') — منصة المئة</title>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div class="relative" x-data="{ open: false }">
                        ...
                        <div x-show="open" @click.away="open = false" class="absolute top-full left-0 mt-2 w-56 bg-white shadow-xl rounded-xl border border-gray-100 py-2 z-50">
                            ...
                            @endphp
                            <a href="{{ $roleHome }}" class="flex items-center gap-3 px-4 py-2 text-sm text-gray-600 hover:bg-amber-50 hover:text-amber-700 font-medium">لوحة التحكم</a>
                            @if(Auth::user()->hasRole('student'))
                                <a href="{{ route('student.profile') }}" class="flex items-center gap-3 px-4 py-2 text-sm text-gray-600 hover:bg-amber-50 hover:text-amber-700 font-medium">الملف الشخصي</a>
                            @endif
        ...
        <aside id="sidebar" class="fixed lg:static inset-y-0 right-0 z-30 w-64 bg-white border-l border-gray-100 shadow-sm pt-16 sm:pt-20 lg:pt-0 transform transition-transform duration-300 -translate-x-full lg:translate-x-0 overflow-y-auto">
            <nav class="p-4 space-y-1 mt-4">
                ...
                @foreach($sidebarItems as $key => $item)
                    ...
                    @endphp
                    <a href="{{ $itemRoute ?? (route($sidebarRoute) . '?tab=' . $key) }}"
    ...
    <script>
        function notifBell() {
            return {
                ...
                fetchDropdown() {
                    fetch('{{ route('student.notifications.dropdown') }}', {
                        headers: { 'Accept': 'application/json' }
                    }).then(r => r.json()).then(d => {
