...
@section('content')
<div class="bg-white" x-data="{ tab: 'overview', showPaymentModal: false, processing: false, isLoggedIn: {{ $user ? 'true' : 'false' }} }">
            ...
            <div class="grid lg:grid-cols-3 gap-8 items-start">
                <div class="lg:col-span-2">
                    <div class="flex items-center gap-2 text-sm text-blue-200 mb-3">
                        @if($course->subject)
                            <span class="px-3 py-1 rounded-full bg-white/10 text-blue-100 font-bold">{{ $course->subject->name_ar ?: $course->subject->name }}</span>
                        @endif
                        @if($course->difficulty_level)
                            <span class="px-3 py-1 rounded-full bg-white/10">{{ ['beginner' => 'مبتدئ', 'intermediate' => 'متوسط', 'advanced' => 'متقدم'][$course->difficulty_level] ?? $course->difficulty_level }}</span>
                        @endif
                    </div>
                    <h1 class="text-3xl sm:text-4xl lg:text-5xl font-black mb-4 leading-tight">{{ $course->title_ar ?: $course->title }}</h1>
                    @if($course->short_description)
                        <p class="text-blue-200 text-lg leading-relaxed mb-6">{{ $course->short_description }}</p>
                    @elseif($course->description_ar ?? $course->description)
                        <p class="text-blue-200 text-lg leading-relaxed mb-6 line-clamp-3">{{ $course->description_ar ?: $course->description }}</p>
                    @endif
                    <div class="flex flex-wrap items-center gap-4 text-sm text-blue-200">
                        @if($instructor)
                            <span class="flex items-center gap-2">
                                <span class="w-8 h-8 rounded-full bg-amber-400 flex items-center justify-center text-blue-900 font-bold text-xs">{{ mb_substr($instructor->name, 0, 1) }}</span>
                                {{ $instructor->name }}
                            </span>
                        @endif
                        <span><span class="font-bold text-white">{{ $totalLessons }}</span> دروس</span>
                        @if($course->duration_minutes)
                            <span><span class="font-bold text-white">{{ ceil($course->duration_minutes / 60) }}</span> ساعة</span>
                        @endif
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl overflow-hidden shadow-xl">
                    ...
                    @else
                        <div class="w-full h-48 bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center text-white text-5xl font-black">{{ mb_substr($course->title, 0, 2) }}</div>
                    @endif
                    <div class="p-6 text-center">
                        <div class="text-3xl font-black text-gray-900 mb-2">
                            {{ $course->price > 0 ? number_format($course->price, 0) . ' ريال' : 'مجاني' }}
                        </div>
                        @if($hasAccess)
                        ...
                        @else
                            <button @click="isLoggedIn ? (window.location = '{{ route('student.course-detail', $course->id) }}') : document.getElementById('login-trigger')?.click()"
                               class="block w-full bg-emerald-500 hover:bg-emerald-600 text-white py-3 rounded-xl font-bold text-lg transition-colors mb-3">
                                بدء التعلم مجاناً
                            </button>
                        @endif
                        <ul class="space-y-2 text-right text-sm text-gray-500">
                            ...
                            <li class="flex items-center gap-2">
                                <svg class="w-4 h-4 text-emerald-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                                @if($course->has_certificate) شهادة إتمام @else تدريب شامل @endif
                            </li>
                        </ul>
                    </div>
                </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div x-show="tab === 'overview'" x-transition>
            @if($course->description_ar ?? $course->description)
                <div class="bg-white border border-gray-100 rounded-3xl p-6 sm:p-8 shadow-sm mb-6">
                    ...
                    <p class="text-gray-600 leading-relaxed whitespace-pre-line">{{ $course->description_ar ?: $course->description }}</p>
                </div>
            @endif
            @if($course->short_description)
        ...
        <div x-show="tab === 'syllabus'" x-transition>
            <div class="bg-white border border-gray-100 rounded-3xl p-6 sm:p-8 shadow-sm">
                ...
                <p class="text-gray-500 text-sm mb-6">{{ $totalLessons }} دروس موزعة على {{ $course->modules->count() }} أقسام</p>
                                ...
                                <div class="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 transition-colors">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        @if($lesson->duration_minutes)
                                            <p class="text-xs text-gray-400">{{ $lesson->duration_minutes }} دقيقة</p>
                                        @endif
                                    </div>
                                    @if($lesson->is_free)
            ...
            <div x-show="tab === 'instructor'" x-transition>
                <div class="bg-white border border-gray-100 rounded-3xl p-6 sm:p-8 shadow-sm">
                    <div class="flex items-center gap-4 mb-6">
                        <div class="w-16 h-16 rounded-full bg-gradient-to-br from-amber-400 to-amber-500 flex items-center justify-center text-white font-black text-xl shadow-sm">{{ mb_substr($instructor->name, 0, 1) }}</div>
                        <div>
                            <h2 class="text-xl font-black text-gray-900">{{ $instructor->name }}</h2>
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="grid md:grid-cols-3 gap-6">
                    @foreach($relatedCourses as $rc)
                        @php $rcLessons = $rc->modules->sum(fn($m) => $m->lessons->count()); @endphp
                        <a href="{{ route('course-detail', $rc->id) }}" class="group bg-white border border-gray-200 rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all hover:-translate-y-1">
                            <div class="h-36 bg-gradient-to-br {{ $loop->even ? 'from-amber-400 to-orange-500' : 'from-blue-500 to-indigo-600' }} flex items-center justify-center text-white text-2xl font-black relative">
                                @if($rc->thumbnail)
                                    <img src="{{ $rc->thumbnail }}" alt="" class="absolute inset-0 w-full h-full object-cover" loading="lazy">
                                @endif
                                {{ mb_substr($rc->title_ar ?? $rc->title, 0, 2) }}
                            </div>
                            <div class="p-4">
                                <h3 class="font-bold text-gray-900 group-hover:text-amber-600 transition-colors text-sm">{{ $rc->title_ar ?? $rc->title }}</h3>
                                <p class="text-xs text-gray-400 mt-2">{{ $rcLessons }} درس</p>
                            </div>
                        </a>
                    @endforeach
                </div>
            ...
            <div class="text-center">
                ...
                <div class="bg-gray-50 rounded-2xl p-4 mb-6 text-right">
                    <div class="flex justify-between items-center py-2">
                        <span class="text-gray-600">{{ $course->title_ar ?? $course->title }}</span>
                        <span class="font-bold text-gray-900">{{ number_format($course->price, 0) }} ريال</span>
                    </div>
                    <div class="border-t border-gray-200 pt-2 flex justify-between items-center">
                        ...
                        <span class="font-bold text-amber-600 text-lg">{{ number_format($course->price, 0) }} ريال</span>
                    </div>
                </div>
                <form action="{{ route('student.payment-request') }}" method="POST">
                    @csrf
