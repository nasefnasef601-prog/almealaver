...
<div class="max-w-6xl" x-data="{ search: '' }">
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="{{ route('courses') }}" class="text-sm font-bold text-blue-600 hover:text-blue-800">تصفح الكورسات ←</a>
    </div>
    ...
    @if($courses->isEmpty())
        <div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-12 text-center">
            ...
            <a href="{{ route('courses') }}" class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-bold transition-colors">
    ...
    @else
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($courses as $course)
                ...
                @endphp
                <div x-show="search === '' || '{{ str_replace("'", "\'", $searchText) }}'.includes(search)"
                     x-transition
                    ...
                    <div class="h-40 bg-gradient-to-br {{ $loop->even ? 'from-amber-400 to-orange-500' : 'from-blue-500 to-indigo-600' }} flex items-center justify-center text-white text-2xl font-black relative">
                        ...
                        @endif
                        <span class="drop-shadow-lg">{{ mb_substr($course->title_ar ?? $course->title, 0, 2) }}</span>
                    </div>
                    <div class="p-5">
                        <h3 class="font-bold text-gray-900 mb-1 group-hover:text-amber-600 transition-colors">{{ $course->title_ar ?? $course->title }}</h3>
                        @if($course->subject)
                            <p class="text-xs text-gray-400 mb-3">{{ $course->subject->name_ar ?? $course->subject->name }}</p>
                        @endif
                        <div class="flex items-center gap-2 mb-4">
                            <div class="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
                                <div class="h-full rounded-full {{ $isComplete ? 'bg-emerald-500' : 'bg-blue-500' }}" style="width: {{ $progress }}%"></div>
                            </div>
                            <span class="text-xs font-bold {{ $isComplete ? 'text-emerald-600' : 'text-gray-500' }}">{{ $progress }}%</span>
                        </div>
                        <a href="{{ route('student.course-detail', $course->id) }}" class="block text-center py-2.5 rounded-xl font-bold text-sm transition-colors {{ $isComplete ? 'bg-emerald-500 hover:bg-emerald-600 text-white' : 'bg-blue-600 hover:bg-blue-700 text-white' }}">
                            @if($isComplete) عرض الكورس @elseif($progress > 0) متابعة التعلم @else بدء التعلم @endif
                        </a>
                    </div>
                </div>
            @endforeach
